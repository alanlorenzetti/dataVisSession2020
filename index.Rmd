...
**3.** Click on `Style` tab. Let's change the design of our network.
  ...
  + Change the shapes of nodes according to the functional `class` of proteins. Rectangles will represent `Transcription` class. Diamonds will represent `Translation` class.
  + Add a border to nodes of proteins used as baits. Set size `2`.
  + Make the nodes bigger. Select size `35`.
  + Click on `Node` tab and make the edges wider (size `1.5`) and `black`.
